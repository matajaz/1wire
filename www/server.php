...
//		echo $jsonstr;
		...
		for ( $counter = 0; $counter < count($queriesarray); $counter ++) {
			...
			$logging = write_log("QUERY: stime=$stime, counter=$counter, $query");
...
//				$datum = $raw['Timestamp'].substr(0,9);	//really slow
//				$year = substr($datum,0,4);		//fast
//				$month = substr($datum,5,2);	//fast
//				$day = substr($datum,8,2);		//fast
//				$tid = substr($raw['Timestamp'],-8); //slow
				$strang = number_format($raw['Temperature'],1) . ",";
				$jsonstr = json_encode($strang);
				echo $jsonstr;
//				$i++;
			} // end while
...
//		$query1 = $queryobj->query1;
//		$query2 = $queryobj->query2;
//		$query3 = $queryobj->query3;
//echo "query1='$query1' <br />query2='$query2' <br />query3='$query3'<br />";
//		$Temp1 = mysql_query($query1) or die("Query failed with error: ".mysql_error() . " Query = " . $query1);
//		$Temp2 = mysql_query($query2) or die("Query failed with error: ".mysql_error() . " Query = " . $query2);
//		$Temp3 = mysql_query($query3) or die("Query failed with error: ".mysql_error() . " Query = " . $query3);
/*		$i = 0;
		$rows = mysql_num_rows($Temp1) or die(mysql_error());
		While ($raw = mysql_fetch_array ($Temp1,MYSQL_ASSOC)){
			$datum = $raw['Timestamp'].substr(0,9);
			$year = substr($datum,0,4);
			$month = substr($datum,5,2);
			$day = substr($datum,8,2);
			$tid = substr($raw[$col1],-8);
//			$strang = "$month/$day/$year $tid" . "," . number_format($raw[$col2],1) . ",";
//			$strang = $raw[$col1] . "," . $raw[$col2] . ",";
			$strang = number_format($raw['Temperature'],1) . ",";
			$jsonstr = json_encode($strang);
			echo $jsonstr;
			$i++;
